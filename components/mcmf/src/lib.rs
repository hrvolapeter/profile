    ...
    T: Clone + Ord,
{
    ...
    pub fn vertices(&self) -> Vec<&Vertex<T>> {
        iter::once(&self.flows[0].a).chain(self.flows.iter().map(|x| &x.b)).collect()
    }
    ...
    pub fn cost(&self) -> i64 {
        self.flows.iter().map(|flow| flow.amount as i64 * flow.cost).sum()
    }
    ...
    T: Clone + Ord,
{
    ...
    pub fn mcmf(&self) -> (i64, Vec<Path<T>>, Vec<Flow<T>>) {
        let mut next_id = 0;
        ...
        let total_amount = g.mcmf() as i64;
        ...
        let flows: Vec<_> = edges
            ...
            .map(|x| {
                ...
                let cost = x.data.cost as i64;
                Flow { a, b, amount, cost }
            })
            .collect();
        let mut paths = GraphBuilder::path_decomposition(flows.clone());
        paths.sort_by_key(|path| path.len());
        (total_amount, paths, flows)
    }
        ...
        fn decompose<T: Clone + Ord>(
            ...
            if *v == Vertex::Sink {
                ...
                if amount < flow.amount {
                    adj.get_mut(&v)
                        .unwrap()
                        .push(Flow { amount: flow.amount - amount, ..flow.clone() });
                }
...
mod tests {
    ...
    fn large_number() {
        ...
        for i in 0..48 {
            let x = i * 1000;
            println!("x={}", x);
            let (total, _, _) = GraphBuilder::new()
                .add_edge(Vertex::Source, OnlyNode::Only, Capacity(x), Cost(x))
                .add_edge(Vertex::Source, OnlyNode::Only, Capacity(x), Cost(x))
                .add_edge(OnlyNode::Only, Vertex::Sink, Capacity(x), Cost(0))
                .mcmf();
            assert_eq!(total, (x as i64 * x as i64) as i64);
        }
    }

    #[test]
    fn empty_graph() {
        let (cost, paths, _) = GraphBuilder::<i64>::new().mcmf();
        assert_eq!(cost, 0);
    ...
    fn large_capacities() {
        ...
        let (cost, paths, _) = GraphBuilder::new()
